...
class TransferGatlingSpec extends Simulation with CommonGatling {
  ...
  val scn: ScenarioBuilder = scenario("simple transfer scenario")
    ...
    .exec(
      ...
      .body(StringBody("""{ "fromId":${createdFromId},
                       |  "toId": ${createdToId},
                       |  "money": 100
                       |}""".stripMargin))
